...
abstract class Amphipod {
	...
	*movesFromStart(state: State): Generator<[number, Amphipod]> {
	...
	moveFromHall(state: State): [number, Amphipod] | undefined {
		...
		if (
			room
				...
				.some(
					(amphipod) => amphipod!.position !== AmphipodPosition.Stop
				)
		) {
			return;
		}
		...
		const y = this.findLastIndex(room, (amphipod) => !amphipod) + 1;
	...
	findLastIndex(room: (Amphipod | undefined)[], predicate: (amphipod: Amphipod | undefined) => boolean): number {
	...
	*[Symbol.iterator]() {
		for (const room of Object.values(this.rooms)) {
			for (const amphipod0 of room) {
				...
				for (const [cost, amphipod1] of amphipod0.movesFromStart(
					this
				)) {
	...
	heuristic(): number {
		...
		let totalHeuristic = Object.values(this.halls).reduce(
			(sum, amphipod) => {
		...
		Object.values(this.rooms).forEach((room, x) => {
			...
			room.forEach((amphipod, y) => {
				if (amphipod) {
					...
					totalHeuristic +=
						(y + 1) * amphipod.energy + amphipod.heuristicX();
				} else {
...
class Day23 {
	...
	public solveForFirstStar(lines: string) {
		const startingState = State.from(lines);
		return this.minimumEnergy(startingState, 50_000);
	}

	public solveForSecondStar(lines: string[]) {
		lines.splice(3, 0, "  #D#C#B#A#  ", "  #D#B#A#C#  ");
		const startingState = State.from(lines.join("\n"));
	...
	private minimumEnergy(startingState: State, openHeapLength: number): number {
		...
		const openHeap: State[][] = Array.from(
			{ length: openHeapLength },
			() => []
		);
